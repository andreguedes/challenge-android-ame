...
class ProductDetailActivityTest {
    ...
    fun verifyTextsOnViews() {
        ...
        val description = StringHelper.formatHTML(product.description)
        onView(withId(R.id.txt_product_description)).check(matches(withText(description.toString())))

        val priceFrom = StringHelper.formatCurrencyNew(product.priceFrom)
        onView(withId(R.id.txt_price_from)).check(matches(withText("De: $priceFrom")))

        val priceTo = StringHelper.formatCurrencyNew(product.priceTo)
